...
export default function Nav(_props: NavProps = {}) {
  ...
  return (
    <>
      ...
      <Link
        href="/store/cart"
        className="fixed top-4 right-16 z-50 p-2 rounded-full hover:bg-white/10 transition-colors group"
        title="Cart"
      >
        <svg
        ...
        {cartCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 bg-[#f97316] text-black text-[10px] font-bold w-[18px] h-[18px] rounded-full flex items-center justify-center">
      ...
      <Link 
        href={isLoggedIn ? "/account/dashboard" : "/account"} 
        className="fixed top-4 right-6 z-50 p-2 rounded-full hover:bg-white/10 transition-colors group"
        title="Account"
      >
        <div className="relative">
          <svg 
            className={`w-6 h-6 transition-colors ${isLoggedIn ? "text-[#f97316] group-hover:text-[#fb923c]" : "text-gray-400 group-hover:text-white"}`}
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
          ...
          {isLoggedIn && (
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 rounded-full border border-[#030305]" />
          )}
        </div>
      </Link>
